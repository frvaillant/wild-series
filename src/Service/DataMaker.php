...
class DataMaker
{
    ...
    public static function slugMaker(string $datum): ?string
    {
        return strtolower(str_replace(' ', '', str_replace('\'', '', $datum)));
    }

    public static function getPicture(): string
    {
        $defaultPicture = 'https://apod.nasa.gov/apod/image/1910/MilkyWaySaltFlat_Huerta_960.jpg';
        $date           = rand(2017, 2019) . '-' . rand(1, 12) . '-' . rand(1, 28);
        $key            = '0fXozWA7A12bdIHnys6SNfMIyTcW6wMuQgqvul0p';
        $url            = 'https://api.nasa.gov/planetary/apod?api_key=' . $key . '&date='.$date;
        $picture        = json_decode(file_get_contents($url), true);
        $extension      = substr($picture['url'],-3);
        if ($extension === 'jpg') {
            return $picture['url'];
        } else {
